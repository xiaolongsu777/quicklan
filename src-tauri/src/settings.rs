...
impl SettingsService {
    ...
    pub fn update_avatar(
        ...
        let avatar_path = avatar_path.as_ref();
        {
            let mut settings = self
                .inner
                .lock()
                .map_err(|_| "设置正在被占用".to_string())?;
            settings.avatar_path = Some(avatar_path.display().to_string());
            settings.avatar_hash = Some(avatar_hash);
        }
        self.save()?;
        Ok(self.get())
    }
